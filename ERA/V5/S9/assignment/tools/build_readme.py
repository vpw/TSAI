...
The point is that no number in the write-up is typed by hand: every one is looked up from
the JSON the notebook wrote on its last run. A placeholder that does not resolve is an
error, not a silently-empty span.
"""
