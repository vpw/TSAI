...
about {{meta.total_runtime_seconds:.0f}} seconds. Every number in this README is substituted
...
{{section12.max_abs_error_gib:.2f}} GiB.

## Results

Measured at the assignment's world size of **{{section9.world_size}} virtual GPUs**, on
nanoGPT ({{section4.n_params:,}} parameters), over {{section9.steps}} steps of
{{section9.global_batch_tokens:,}} tokens each:

| arrangement | bytes/weight | vs DP | communication | 30B @ 32 GPUs | fits a 74.5 GiB card? |
| --- | --- | --- | --- | --- | --- |
| data parallel | {{section9.measured.dp.bytes_per_weight:.4f}} | {{section9.measured.dp.vs_dp:.1f}}x | {{section9.measured.dp.comm_multiple_of_P:.4f}}P | {{section12.projected.dp.32:.1f}} GiB | no, at any world size |
| ZeRO-1 | {{section9.measured.zero1.bytes_per_weight:.4f}} | {{section9.measured.zero1.vs_dp:.1f}}x | {{section9.measured.zero1.comm_multiple_of_P:.4f}}P | {{section12.projected.zero1.32:.1f}} GiB | no, at any world size |
| ZeRO-2 | {{section9.measured.zero2.bytes_per_weight:.4f}} | {{section9.measured.zero2.vs_dp:.1f}}x | {{section9.measured.zero2.comm_multiple_of_P:.4f}}P | {{section12.projected.zero2.32:.1f}} GiB | yes, from {{section11.fits_from.zero2}} GPUs |
| ZeRO-3 | {{section9.measured.zero3.bytes_per_weight:.4f}} | {{section9.measured.zero3.vs_dp:.1f}}x | {{section9.measured.zero3.comm_multiple_of_P:.4f}}P | {{section12.projected.zero3.32:.1f}} GiB | yes, from {{section11.fits_from.zero3}} GPUs |
...
> over all {{section10.steps}} steps. Largest weight difference against data parallelism:
> **{{section10.equivalence.zero3.max_weight_diff}}**.

ZeRO moved {{section10.state_removed_fraction:.1%}} of the training state off each GPU and
...
| **total** | **{{section1.bytes_per_weight}}** | |

At V5's 30 billion parameters that is **{{section1.v5_state_gib:.1f}} GiB** — about
{{section1.cards_needed}} cards of {{section1.card_gib:.1f}} GiB each, before a single
...
**{{section1.bf16_demo.lost_fraction:.0%}} of the training signal is lost** without the
...
At N=32 that is **{{section3.all_reduce_multiple_of_P:.4f}}P**, not 2P. The lesson's 2P is
...
**{{section3.identity_max_abs_diff}}**, i.e. bit-identical, not merely close.
...
**{{section3.gloo.world_size}} real `torch.distributed` processes on the gloo backend** and
compares their output tensor by tensor. Largest disagreement:
**{{section3.gloo.max_diff_all_reduce:.1e}}** — float32 rounding, not a difference in
...
seq_len=128`, **{{section4.n_params:,}} parameters** in {{section4.n_tensors}} tensors
...
Each rank owns **{{section4.shard_numel:,}}** parameters. Every group happens to divide
...
{{section9.steps}} steps, was **{{section9.measured.dp.max_replica_gap}}**. They are
...
{{section9.measured.zero1.comm_multiple_of_P:.4f}}P against data parallelism's
{{section9.measured.dp.comm_multiple_of_P:.4f}}P — because of §4's identity: data parallelism
...
from 2 bytes per weight to `2/N`. Still {{section9.measured.zero2.comm_multiple_of_P:.4f}}P.
...
{{section9.measured.zero3.comm_multiple_of_P:.4f}}P comes from.
...
billion weights is **{{section11.replicated_floor_gib:.1f}} GiB** regardless of how many
cards there are. Adding GPUs divides the sharded part and does exactly nothing to the
replicated part, so those two lines approach a floor above the {{section11.card_gib:.1f}} GiB
...
**{{section11.boundary_params_b:.0f}}B parameters**. A 20B model is on the line; V5's 30B is
past it. That single number is why the session concludes the choice is ZeRO-2 or ZeRO-3 and
nothing below.

ZeRO-2 fits from {{section11.fits_from.zero2}} GPUs, ZeRO-3 from
{{section11.fits_from.zero3}} — matching §7 exactly.
...
**{{section13.h100_bucket2_hidden_fraction:.1%}}**, and the reason is pleasingly simple once
...
{{section13.b200_alpha_threshold_ms:.1f}} ms**, and the lesson's claim holds in this model for
...
{{section13.comm_fraction_h100:.0%}} on H100 and {{section13.comm_fraction_b200:.0%}} on
...
At 8 GPUs, ZeRO-2 needs {{section14.rows.8.gpu_gib_no_offload:.1f}} GiB against a
{{section11.card_gib:.1f}} GiB card. Offload brings it to
{{section14.rows.8.gpu_gib_offloaded:.1f}} GiB for
{{section14.rows.8.pcie_gb_per_step:.2f}} GB of PCIe traffic per step —
{{section14.rows.8.pcie_seconds:.3f}}s at {{section14.pcie_gbps:.0f}} GB/s. The lesson's
...
byte per block of 32 values across both tensors — {{section15.scale_overhead_bytes_per_weight}}
bytes per weight. Net: **{{section15.mxfp8_bytes_per_weight:.4f}}** bytes per weight, a
reduction of only **{{section15.reduction_fraction:.1%}}** (the lesson's 12.1%), or
{{section15.bf16_30b_gib:.1f}} → {{section15.mxfp8_30b_gib:.1f}} GiB at 30B.
...
between any two of the 32 copies was {{section9.measured.dp.max_replica_gap}} across all
{{section9.steps}} steps, which is another way of saying 31 of them were redundant. At 30B it
needs
{{section12.projected.dp.32:.1f}} GiB per card against {{section11.card_gib:.1f}}, and
**adding GPUs does not help at all**.

**Use it when** the model comfortably fits — for a 74.5 GiB card at 16 bytes/weight, roughly
{{section11.dp_max_params_b:.0f}}B parameters or fewer.

### ZeRO-1

**For:** free. Measured {{section9.measured.zero1.comm_multiple_of_P:.4f}}P against data
parallelism's {{section9.measured.dp.comm_multiple_of_P:.4f}}P — identical, because of §4's
identity.

**Against:** it helps least where help is most needed. Weights and gradients stay replicated,
so it is trapped behind the same {{section11.replicated_floor_gib:.1f}} GiB floor and **never
fits a large model at any world size**. Its {{section9.measured.zero1.vs_dp:.1f}}x saving is real and lands on the wrong side
of the card.
...
{{section9.measured.zero2.bytes_per_weight:.4f}} bytes/weight against
{{section9.measured.dp.bytes_per_weight:.4f}} — **{{section9.measured.zero2.vs_dp:.1f}}x less state at identical
...
floor of 2 bytes/weight ({{section11.weight_only_floor_gib:.1f}} GiB at 30B) and stops improving however many GPUs are
added. It
needs {{section11.fits_from.zero2}} GPUs before it fits at all. The steady-state figure also
...
`16/N` and keeps falling — {{section9.measured.zero3.bytes_per_weight:.4f}} bytes/weight at
N=32, a {{section9.measured.zero3.vs_dp:.0f}}x reduction. The only option that fits 30B on {{section11.fits_from.zero3}} GPUs,
and the only one whose memory problem can be solved by buying more cards.

**Against:** **1.5x the communication** — {{section9.measured.zero3.comm_multiple_of_P:.4f}}P
against {{section9.measured.zero2.comm_multiple_of_P:.4f}}P, exactly "half as much again",
...
{{section9.measured.zero3.transient_bytes:,}} bytes of peak on this model — comparable to the
...
any world size; ZeRO-2 fits from {{section11.fits_from.zero2}} GPUs, ZeRO-3 from
{{section11.fits_from.zero3}}. So the live question is the one §13 of the lesson leaves open
