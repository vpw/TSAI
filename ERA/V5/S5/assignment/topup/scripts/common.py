...
# S5 top-up slice. The S4 run cleaned 43.45M tokens from hin/tel/eng/asm `data-0` shards.
# The ledger says the Indic lane's verified tier (tier A) is the one under repetition
# pressure at 2.5 epochs, and verified tokens are only useful if they cover the languages
# the model has to serve -- so this pass broadens coverage to the Tier-1 languages S4 never
# touched, and adds a *second* Hindi shard so cross-shard deduplication has something to do.
#
# `claimed` is the language asserted for the slice, which stage 3 refuses to trust. Note
# Odia: Sangraha's folder is `ori`, but fastText emits `or` which maps to `ory`. The claim
# is written as the pipeline's own ISO-639-3 label, not as the folder name.
SOURCES = [
    # key            pool        claimed  file                              token budget
    ("ver-ben", "verified", "ben", "verified/ben/data-0.parquet", 30_000_000),
    ("ver-hin2", "verified", "hin", "verified/hin/data-1.parquet", 30_000_000),
    ("ver-mar", "verified", "mar", "verified/mar/data-0.parquet", 25_000_000),
    ("ver-tam", "verified", "tam", "verified/tam/data-0.parquet", 25_000_000),
    ("ver-kan", "verified", "kan", "verified/kan/data-0.parquet", 20_000_000),
    ("ver-guj", "verified", "guj", "verified/guj/data-0.parquet", 20_000_000),
    ("ver-mal", "verified", "mal", "verified/mal/data-0.parquet", 20_000_000),
    ("ver-pan", "verified", "pan", "verified/pan/data-0.parquet", 15_000_000),
    ("ver-ory", "verified", "ory", "verified/ori/data-0.parquet", 15_000_000),
]
